...
from config import BOT_TOKEN, WEBHOOK_SECRET, WEBHOOK_URL, MAX_DELAY_SECONDS, MAX_GROUPS_PER_PAGE, MAX_REPEATS
...
def start_broadcast_worker(admin_id: int, control_chat_id: int, control_message_id: int):
    ...
    for index in range(repeats):
        ...
        if (index + 1) % 5 == 0 or (index + 1) == repeats:
            try:
                bot.edit_message_text(
                    chat_id=control_chat_id,
                    message_id=control_message_id,
...
def configure_webhook():
    if not BOT_TOKEN or not WEBHOOK_URL:
        ...
        return
    webhook = f"{WEBHOOK_URL}/webhook/{WEBHOOK_SECRET}"
    try:
        bot.remove_webhook()
    except Exception:
        pass
