...
LANGUAGE_BUTTONS = [('English', 'en'), ('العربية', 'ar'), ('Русский', 'ru'), ('Français', 'fr'), ('Español', 'es'), ('Deutsch', 'de'), ('Português', 'pt'), ('Italiano', 'it'), ('Türkçe', 'tr'), ('فارسی', 'fa'), ('Bahasa Indonesia', 'id')]
...
def display_user_name(row):
    ...
    return escape(str(first_name or "Unknown"))
...
def build_main_keyboard(lang):
    ...
    rows = [
        ...
        [make_button("📊 Stats", callback_data="show:stats"), make_button("📈 Growth", callback_data="show:growth")],
        [make_button("ℹ️ Info", callback_data="show:info"), make_button("🌍 Language", callback_data="show:language")],
        [make_button("👨‍💻 Creator", callback_data="show:creator")],
    ]
    if group_url:
...
def build_info_text(chat_id, lang):
    ...
    labels_map = {
        "en": {
            ...
            "top_replied": "Most replied",
            "top_reacted": "Most reacted",
            "trend": "Vs yesterday",
            "support": "Support",
            "creator": "Creator",
            "enabled": "Enabled",
        ...
        "ar": {
            ...
            "active_users": "المستخدمون النشطون اليوم",
            "new_users": "المستخدمون الجدد اليوم",
            ...
            "peak_hour": "ساعة الذروة",
            "top_member": "أكثر عضو نشاطًا",
            "top_replied": "أكثر رسالة تم الرد عليها",
            "top_reacted": "أكثر رسالة عليها تفاعل",
            "trend": "مقارنة بالأمس",
            "support": "الدعم",
            "creator": "الصانع",
            "enabled": "مفعّل",
        ...
        "ru": {
            ...
            "active_users": "Активные пользователи сегодня",
            ...
            "top_member": "Самый активный участник",
            "top_replied": "Больше всего ответов",
            "top_reacted": "Больше всего реакций",
            "trend": "По сравнению со вчера",
            "support": "Поддержка",
            "creator": "Создатель",
            "enabled": "Включено",
            "disabled": "Выключено",
            "vibe": "Настроение",
            "topics": "Топ темы",
            "avg": "Среднее на активного пользователя",
        },
    }
    ...
    try:
        ...
        prev_messages = get_previous_message_count(chat_id, utc_today())
        joined = get_joined_count(chat_id, utc_today())
        left = get_left_count(chat_id, utc_today())
        net = get_net_growth(chat_id, utc_today())
        messages = safe_int(stats["messages"] if stats else 0)
        active_users = safe_int(stats["active_users"] if stats else 0)
        new_users = safe_int(get_new_users_count(chat_id, utc_today()))
        ...
        report_enabled = labels["enabled"] if (chat and safe_int(chat["daily_report_enabled"], 1)) else labels["disabled"]
        total_users = count_tracked_users()
        vibe = activity_vibe(messages)
        trend = messages - prev_messages
        ...
        if member_result and member_result.get("ok"):
            member_count = safe_int(member_result.get("result"), None)
        ...
        replied = most_replied_message(chat_id, utc_today())
        reacted = most_reacted_message(chat_id, utc_today())
        topics = extract_topics(topic_candidates(chat_id, utc_today()), lang)[:3]
        ...
        lines = [
            ...
            f"💬 <b>{labels['current_chat']}:</b> {escape(str(chat_title))}",
            f"🌍 <b>{labels['chat_language']}:</b> {escape(str(chat_lang.upper()))}",
            "",
            f"👥 <b>{labels['tracked_users']}:</b> {total_users}",
            f"📄 <b>{labels['daily_report']}:</b> {report_enabled}",
            f"💬 <b>{labels['today_messages']}:</b> {messages}",
            f"🟢 <b>{labels['active_users']}:</b> {active_users}",
            f"🆕 <b>{labels['new_users']}:</b> {new_users}",
            f"🆕 <b>{labels['joined']}:</b> {joined}",
            f"🚪 <b>{labels['left']}:</b> {left}",
            f"📈 <b>{labels['net_growth']}:</b> {net:+d}",
            f"↔️ <b>{labels['trend']}:</b> {trend:+d}",
        ...
        if top:
            lines.append(f"🏆 <b>{labels['top_member']}:</b> {display_user_name(top[0])} ({safe_int(top[0]['messages'])})")
        else:
            lines.append(f"🏆 <b>{labels['top_member']}:</b> N/A")

        if replied:
            lines.append(f"💬 <b>{labels['top_replied']}:</b> {display_user_name(replied)} — {safe_int(replied['replies_count'])} replies")
        else:
            lines.append(f"💬 <b>{labels['top_replied']}:</b> N/A")

        if reacted:
            lines.append(f"❤️ <b>{labels['top_reacted']}:</b> {display_user_name(reacted)} — {safe_int(reacted['reactions_count'])} reactions")
        else:
            lines.append(f"❤️ <b>{labels['top_reacted']}:</b> N/A")

        if topics:
            lines.append("")
            lines.append("🔥 <b>{}:</b> {}".format(labels["topics"], ", ".join(f"#{escape(str(x))}" for x in topics)))

        lines.extend([
            "",
            f"📊 <b>{labels['avg']}:</b> {round(messages / active_users, 1) if active_users else 0}",
        ...
        LOG.exception("build_info_text failed")
        fallback_labels = labels_map["en"]
        return "\n".join([
            fallback_labels["title"],
            "",
            f"🤖 <b>{fallback_labels['bot']}:</b> {BOT_NAME}",
            f"🚀 <b>{fallback_labels['version']}:</b> {BOT_VERSION}",
            "",
            "⚠️ Info details are temporarily unavailable.",
        ])


def send_info_card(chat_id, lang, message_id=None):
    text = build_info_text(chat_id, lang)
    markup = build_main_keyboard(lang)
    if message_id is None:
        return send_message(chat_id, text, reply_markup=markup)
    result = edit_message_text(chat_id, message_id, text, reply_markup=markup)
    if not result or not result.get("ok"):
        return send_message(chat_id, text, reply_markup=markup)
    return result
...
def process_callback_query(callback):
    ...
    if data == "show:info":
        answer_callback(cb_id)
        send_info_card(chat_id, lang, message["message_id"])
        return
